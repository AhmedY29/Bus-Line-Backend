...
export const getSignUpService = async (
  name: string,
  role: string,
  email: string,
  password: string
...
export const getDriverSignUpService = async (
  name: string,
  email: string,
  password: string,
  licenseImage: string,
  ...
  const userExist = await User.findOne({ email });
  const driverExist = await Driver.findOne({ email });
  const driverAccount = await Driver.findOne({
  ...
  const cloudinaryVehicleLicenseImage = await cloudinary.uploader.upload(
    busData.vehicleName
  );
  const cloudinaryVehicleImage = await cloudinary.uploader.upload(
    busData.vehicleImage
  );
  const newVehicle = new Vehicle({
    name: cloudinaryVehicleLicenseImage.url,
    color: busData.vehicleColor.trim(),
    model: busData.vehicleModel.trim(),
...
export const editUserService = async (userId: string, updateData: any) => {
  ...
  if (user) {
    ...
    return {
      success: true,
      message: "Update User in successfully",
      user: updateUser,
    };
  }

  if (driver) {
    const updateUser = await Driver.findByIdAndUpdate(userId, updateData, {
      new: true,
    });

    return {
      success: true,
      message: "Update Driver in successfully",
      user: updateUser,
    };
  }
