...
export const getTripsForDriver = async (req: Request, res: Response) => {
  const token = req.headers.authorization;
  const verify = verifyToken(token?.split(" ")[1] as string);
  if (!verify) {
    res
  ...
  try {
    const trips = await getTripsByDriverIdService(req?.params?.driverId);
    res.status(200).json({
      success: true,
      trips,
    });
